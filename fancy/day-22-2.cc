...
int main() {
  ...
  auto add = [=](int64_t a, int64_t b) { return (m + (a + b) % m) % m; };
  auto mul = [=](int64_t a, int64_t b) { return combine(add, 0, a, b); };
  auto pow = [=](int64_t a, int64_t b) { return combine(mul, 1, a, b); };
